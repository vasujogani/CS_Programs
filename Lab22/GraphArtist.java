...
public class GraphArtist extends JFrame
{
	...
	{
		this(graph, Color.BLACK, Color.BLACK, directed);
	}
...
class GraphPanel extends JPanel
{
		...
		if(null != verticies)
		{
			...
			for(int i = 0; i < verticies.size(); i++)
			{
				...
				for(int j = 0; j < neighbors.length; j++)
				{
					...
					int x = v1.getX() * v1.getW();
					int y = v1.getY() * v1.getH();
					
					int x2 = v2.getX() * v2.getW();
					int y2 = v2.getY() * v2.getH();
					
					int xOff = v1.getW()/2;
					int yOff = v1.getH()/2;
					int d = 1;
					
					drawEdge(g, x + xOff, y + yOff, x2 + xOff, y2 + yOff, color, label, i < k);
				}	
			}
			...
			for(int i = 0; i < verticies.size(); i++)
			{
				...
				drawVertex(g, vertex.getX() * vertex.getW(), vertex.getY() * vertex.getH(), vertex.getW(), vertex.getH(), color, vertex.getLabel());
			}
	...
	{
		Color defaultColor = defaultNode;
		IGraphable v = graph.getVertex(index);
		if(v.getLabel().toLowerCase().equals("open"))
		{
			defaultColor = Color.WHITE;
		}
		else if(v.getLabel().toLowerCase().equals("blocked"))
		{
	...
	private void drawVertex(Graphics g, int x, int y, int w, int h, Color c, String label)
	{
		g.setColor(c);
